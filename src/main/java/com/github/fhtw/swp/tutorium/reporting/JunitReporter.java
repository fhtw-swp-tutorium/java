package com.github.fhtw.swp.tutorium.reporting;
...
public class JUnitReporter implements Formatter, Reporter, StrictAware {
    ...
    public JUnitReporter() throws IOException {
        ...
        if (junitResultFileName == null) {
            throw new CucumberException(String.format("System property '%s' not set. No JUnit report will be generated.", JUNIT_RESULT_FILE_NAME_PROPERTY));
        }

        final String junitResultsFolder = JUNIT_RESULTS_FOLDER;

        final Path pathToJunitResultsFolder = Paths.get(".", junitResultsFolder);

        if (!pathToJunitResultsFolder.toFile().exists()) {
            pathToJunitResultsFolder.toFile().mkdir();
        }
